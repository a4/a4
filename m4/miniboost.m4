...
AC_DEFUN([A4_BOOST_CHECK], [
  ...
  if test x$have_miniboost == xyes; then
    ...
    BOOST_SYSTEM
    BOOST_CHRONO
    AC_SUBST([BOOST_LIBS], ["$BOOST_PROGRAM_OPTIONS_LDFLAGS $BOOST_PROGRAM_OPTIONS_LIBS $BOOST_THREAD_LIBS $BOOST_FILESYSTEM_LIBS $BOOST_SYSTEM_LIBS $BOOST_CHRONO_LIBS"])
  fi

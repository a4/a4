...
def fakelibtool_build(task):
	...
	fu = dest.write
	fu("# Generated by libtool - no, that's a lie, by hand!\n")
	if env['vnum']:
		nums = env['vnum'].split('.')
	else:
		nums = "0.0.0".split('.')
...
def apply_link_libtool(self):
	if "shlib" in self.features or "cxxshlib" in self.features:
		linktask = self.link_task
		self.latask = self.create_task('fakelibtool', linktask.outputs, linktask.outputs[0].change_ext('.la'))

	if self.bld.is_install:
		self.bld.install_files('${PREFIX}/lib', linktask.outputs[0].change_ext('.la'), self.env)
...
Task.task_factory('fakelibtool', vars=fakelibtool_vardeps, func=fakelibtool_build, color='BLUE', after="link stlink")

...
InputStreamA4Impl::InputStreamA4Impl(UNIQUE<ZeroCopyStreamResource> in, 
                                 std::string name) {
    _started = false;
    _raw_in = std::move(in);
    _inputname = name;
...
InputStreamA4Impl::~InputStreamA4Impl() {};

bool InputStreamA4Impl::set_error() {
    _error = true;
    _good = false;
    return false;
}

bool InputStreamA4Impl::set_end() {
    _good = false;
    return false;
}

void InputStreamA4Impl::set_hint_copy(bool do_copy) {
    _hint_copy = do_copy;
    if (not _hint_copy) {
...
void InputStreamA4Impl::startup(bool discovery_requested) {
...
bool InputStreamA4Impl::read_header(bool discovery_requested)
{
    ...
    if (!_discovery_complete) {
        if (!_current_metadata_refers_forward) {
            ...
            if (discovery_requested and not discover_all_metadata()) {
                ERROR("a4::io:InputStreamA4Impl - Failed to discover metadata - file corrupted?");
                return set_error();
            }
        }
    }
...
bool InputStreamA4Impl::discover_all_metadata() {
    if (_hint_copy) notify_last_unread_message();
    ...
    while (true) {
        ...
        if (!hmsg->is<StreamHeader>()) {
            ERROR("a4::io:InputStreamA4Impl - Unknown header class!");
            return false;
        }
...
int64_t InputStreamA4Impl::seek_back(int64_t position) {
...
int64_t InputStreamA4Impl::seek(int64_t position) {
    assert(!_compressed_in);
    if (_hint_copy) notify_last_unread_message();
    _coded_in.reset();
...
bool InputStreamA4Impl::carry_metadata(uint32_t& header, int32_t& metadata) {
...
bool InputStreamA4Impl::seek_to(uint32_t header, int32_t metadata, bool carry) {
    drop_compression();
    if (!_discovery_complete) {
        if (seek(0) == -1) {
            ERROR("a4::io:InputStreamA4Impl - Cannot skip in this unseekable stream!");
            set_error();
            return false;
        }
    ...
    if (carry and not carry_metadata(header, metadata)) { // modifies header and metadata
        if (not carry) {
            ERROR("a4::io:InputStreamA4Impl - Attempt to seek to nonexistent metadata!");
        }
        return false;
    }
...
bool InputStreamA4Impl::start_compression(const StartCompressedSection& cs) {
    assert(!_compressed_in);
    if (_hint_copy) notify_last_unread_message();
    _coded_in.reset();
...
void InputStreamA4Impl::drop_compression() {
    if(!_compressed_in) 
        return;
...
bool InputStreamA4Impl::stop_compression(const EndCompressedSection& cs) {
    assert(_compressed_in);
    if (_hint_copy) notify_last_unread_message();
    _coded_in.reset();
...
void InputStreamA4Impl::reset_coded_stream() {
    if (_hint_copy) notify_last_unread_message();
    _coded_in.reset();
    if (_compressed_in) {
        _coded_in.reset(new CodedInputStream(_compressed_in.get()));
